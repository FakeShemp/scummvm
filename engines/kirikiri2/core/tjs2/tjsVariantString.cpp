	...
	{ // thread-protected
		...
		if (TJSStringHeapList) {
			...
			do {
				...
				block_ind++;
				if (block_ind >= TJSStringHeapList->size())
					block_ind = 0;
				count++;
		...
		if (free_block_count) {
			tTJSVariantString **newlist =
				new tTJSVariantString *[(TJSStringHeapList->size() - free_block_count) *
										HEAP_CAPACITY_INC];
...
tTJSVariantString *TJSAllocVariantString(const tjs_char *ref1, const tjs_char *ref2) {
	...
	if (len1 + len2 > TJS_VS_SHORT_LEN) {
		...
		if (ref1)
			wcscpy(ret->LongString, ref1);
		if (ref2)
			wcscpy(ret->LongString + len1, ref2);
	} else {
		if (ref1)
			wcscpy(ret->ShortString, ref1);
		if (ref2)
			wcscpy(ret->ShortString + len1, ref2);
	}
	...
	type_char:
		...
		case TJS_W('X'): {
			...
			if (!width_ind && !prec_ind) {
				...
				if (width_ind && ind[0] + prec > 900)
					goto error;
				if (prec_ind && ind[0] + width > 900)
					goto error;
				if (in >= numparams)
					TJS_eTJSVariantError(TJSBadParamCount);
				tTVInteger integer = (params[in++])->AsInteger();
