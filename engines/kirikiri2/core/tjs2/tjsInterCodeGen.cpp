...
#include "kirikiri2/core/tjs2/tjs.tab.h"
#include "kirikiri2/core/tjs2/tjsConstArrayData.h"
...
int _yyerror(const tjs_char *msg, void *pm, tjs_int pos) {
	...
	if (!TJS_strncmp(msg, TJS_W("syntax error"), 11)) {
		str = TJSSyntaxError;
...
static tjs_int TJSGetContextHashSize(tTJSContextType type) {
	...
	case ctPropertyGetter:
		return 0;
	case ctClass:
		return TJS_NAMESPACE_DEFAULT_HASH_BITS;
...
tTJSInterCodeContext::tTJSInterCodeContext(tTJSInterCodeContext *parent,
										   const tjs_char *name, tTJSScriptBlock *block, tTJSContextType type) : inherited(TJSGetContextHashSize(type)), Properties(NULL) {
...
tTJSInterCodeContext::tTJSInterCodeContext(tTJSScriptBlock *block, const tjs_char *name, tTJSContextType type,
										   ...
										   tSourcePos *srcPos, tjs_int srcPosSize, std::vector<tjs_int> &superpointer) : inherited(TJSGetContextHashSize(type)), Properties(NULL) {
	inherited::CallFinalize = false;
	Parent = NULL;
	PropGetter = PropSetter = SuperClassGetter = NULL;
...
void tTJSInterCodeContext::Finalize(void) {
	if (PropSetter)
		PropSetter->Release(), PropSetter = NULL;
	if (PropGetter)
		PropGetter->Release(), PropGetter = NULL;
	if (SuperClassGetter)
		SuperClassGetter->Release(), SuperClassGetter = NULL;

	if (CodeArea)
		TJS_free(CodeArea), CodeArea = NULL;
	if (_DataArea) {
	...
	if (SourcePosArray)
		TJS_free(SourcePosArray), SourcePosArray = NULL;
...
const tjs_char *tTJSInterCodeContext::GetContextTypeName() const {
	switch (ContextType) {
	case ctTopLevel:
		return TJS_W("top level script");
	case ctFunction:
		return TJS_W("function");
	case ctExprFunction:
		return TJS_W("function expression");
	case ctProperty:
		return TJS_W("property");
	case ctPropertySetter:
		return TJS_W("property setter");
	case ctPropertyGetter:
		return TJS_W("property getter");
	case ctClass:
		return TJS_W("class");
	case ctSuperClassGetter:
		return TJS_W("super class getter proxy");
	default:
...
tjs_int tTJSInterCodeContext::CodePosToSrcPos(tjs_int codepos) const {
	...
	while (true) {
		if (e - s <= 1)
			return SourcePosArray[s].SourcePos;
		tjs_uint m = s + (e - s) / 2;
...
tjs_int tTJSInterCodeContext::GenNodeCode(tjs_int &frame, tTJSExprNode *node,
	...
	case T_LOGICALAND: // '&&' operator
	{
		...
		tjs_int addr1 = CodeAreaSize;
		AddJumpList();
		PutCode(node->GetOpecode() == T_LOGICALOR ? (inv ? VM_JNF : VM_JF) : (inv ? VM_JF : VM_JNF), node_pos);
			...
			if (n->GetOpecode() == T_EXPANDARG) {
				...
				if ((*n)[0])
					AddFuncArg(_GenNodeCode(
								   frame, (*n)[0], TJS_RT_NEEDED, 0, tSubParam()),
							   fatExpand);
				else
					AddFuncArg(0, fatUnnamedExpand);
			} else {
				AddFuncArg(_GenNodeCode(
							   frame, (*node)[0], TJS_RT_NEEDED, 0, tSubParam()),
						   fatNormal);
			}
		} else {
...
void tTJSInterCodeContext::ExitForCode() {
	...
	PutCode(NestVector.back().LoopStartIP - jmp_ip, LEX_POS);
	DoNestTopExitPatch();
	if (NestVector.back().VariableCreated)
		ExitBlock();
	DoNestTopExitPatch();
	NestVector.pop_back();
}
...
void tTJSInterCodeContext::EnterSwitchCode(tTJSExprNode *node) {
	...
	if (fr - 1 > MaxFrameCount)
		MaxFrameCount = fr - 1;

	FrameBase++; // increment FrameBase
	if (FrameBase - 1 > MaxFrameCount)
		MaxFrameCount = FrameBase - 1;
...
void tTJSInterCodeContext::EnterTryCode() {
	...
	if (FrameBase > MaxFrameCount)
		MaxFrameCount = FrameBase;
}
//---------------------------------------------------------------------------
