...
static tTJSSkipCommentResult TJSSkipComment(const tjs_char **ptr) {
	...
	if ((*ptr)[1] == TJS_W('/')) {
		...
		(*ptr) += 2;
		if (*(*ptr) == 0)
			TJS_eTJSError(TJSUnclosedComment);
		tjs_int level = 0;
		for (;;) {
...
// TJSParseString
//---------------------------------------------------------------------------
enum tTJSInternalParseStringResult { psrNone,
									 psrDelimiter,
									 psrAmpersand,
									 psrDollar };
...
tTJSSkipCommentResult tTJSLexicalAnalyzer::ProcessPPStatement() {
	...
	if (!TJS_strncmp(Current, TJS_W("if"), 2)) {
		...
		if (!ret)
			return SkipUntil_endif(); // skip to endif
...
re_match:
	...
	case TJS_W('9'):
		// number
		{
			...
			if (!r)
				TJS_eTJSCompileError(TJSNumberError, Block, Current - Script);
			n = PutValue(v);
			return T_CONSTVAL;
		}
	}
