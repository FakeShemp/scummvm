...
public:
	Debugger()
		: DebuggerHwnd(INVALID_HANDLE_VALUE), LastLineNo(-1), TypeOfExec(EXEC_RUN), IsInitialConnect(false), StackTraceDepth(10) {
		Initialize();
	}
	void Initialize() {
		...
		if (evtype == DBGHOOK_PREV_BREAK) {
			...
			case EXEC_STEP:
				if (evtype == DBGHOOK_PREV_CALL)
					StepNest++;
				else if (evtype == DBGHOOK_PREV_RETURN)
					StepNest--;
	...
	bool HandleDebuggerMessage() {
		...
		for (int i = 0; cmd && (i < count); i++) {
			...
			case DBGEV_GER_EXEC:
				TypeOfExec = EXEC_RUN;
				return true;
			case DBGEV_GER_BREAK:
				TypeOfExec = EXEC_STOP;
				break;
			case DBGEV_GER_STEP:
				TypeOfExec = EXEC_STEP;
				return true;
			case DBGEV_GER_TRACE:
				TypeOfExec = EXEC_TRACE;
				return true;
			case DBGEV_GER_RETURN:
				TypeOfExec = EXEC_RETURN;
				return true;
