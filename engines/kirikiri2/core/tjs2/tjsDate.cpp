...
tTJSNC_Date::tTJSNC_Date() : tTJSNativeClass(TJS_W("Date")){
								 // class constructor
...
_this->DateTime = time(&curtime); // GMT current date/time
	...
	if (param[0]->Type() == tvtString) {
		...
		if (TJS_PARAM_EXIST(1))
			mon = (tjs_int)param[1]->AsInteger();
		if (TJS_PARAM_EXIST(2))
			day = (tjs_int)param[2]->AsInteger();
		if (TJS_PARAM_EXIST(3))
			h = (tjs_int)param[3]->AsInteger();
		if (TJS_PARAM_EXIST(4))
			m = (tjs_int)param[4]->AsInteger();
		if (TJS_PARAM_EXIST(5))
			s = (tjs_int)param[5]->AsInteger();
		tm t;
