...
#include "kirikiri2/core/tjs2/tjsBinarySerializer.h"
...
//---------------------------------------------------------------------------
class tTJSArraySortCompare_NormalAscending : public std::binary_function<const tTJSVariant &, const tTJSVariant &, bool> {
public:
	result_type operator()(first_argument_type lhs, second_argument_type rhs) const {
		return lhs < rhs;
	}
};
class tTJSArraySortCompare_NormalDescending : public std::binary_function<const tTJSVariant &, const tTJSVariant &, bool> {
public:
	result_type operator()(first_argument_type lhs, second_argument_type rhs) const {
		return lhs > rhs;
	}
};
class tTJSArraySortCompare_NumericAscending : public std::binary_function<const tTJSVariant &, const tTJSVariant &, bool> {
public:
	result_type operator()(first_argument_type lhs, second_argument_type rhs) const {
...
class tTJSArraySortCompare_NumericDescending : public std::binary_function<const tTJSVariant &, const tTJSVariant &, bool> {
public:
	result_type operator()(first_argument_type lhs, second_argument_type rhs) const {
		if (lhs.Type() == tvtString && rhs.Type() == tvtString) {
			tTJSVariant ltmp(lhs), rtmp(rhs);
...
class tTJSArraySortCompare_StringAscending : public std::binary_function<const tTJSVariant &, const tTJSVariant &, bool> {
public:
	result_type operator()(first_argument_type lhs, second_argument_type rhs) const {
		if (lhs.Type() == tvtString && rhs.Type() == tvtString)
			return lhs < rhs;
...
class tTJSArraySortCompare_StringDescending : public std::binary_function<const tTJSVariant &, const tTJSVariant &, bool> {
public:
	result_type operator()(first_argument_type lhs, second_argument_type rhs) const {
		if (lhs.Type() == tvtString && rhs.Type() == tvtString)
			return lhs > rhs;
...
class tTJSArraySortCompare_Functional : public std::binary_function<const tTJSVariant &, const tTJSVariant &, bool> {
...
tTJSArrayClass::tTJSArrayClass() : tTJSNativeClass(TJS_W("Array")) {
	// class constructor
	...
	TJS_BEGIN_NATIVE_METHOD_DECL(/* func.name */ pop) {
		...
		if (ni->Items.empty()) {
			...
			if (result)
				*result = ni->Items[ni->Items.size() - 1];
	...
	TJS_BEGIN_NATIVE_METHOD_DECL(/* func.name */ find) {
		...
		if (result) {
			...
			if (start < 0)
				start = 0;
			if (start >= (tjs_int)ni->Items.size()) {
				*result = -1;
				return TJS_S_OK;
			}
...
tTJSArrayObject::DeleteMemberByNum(tjs_uint32 flag, tjs_int num,
	...
	if (num < 0)
		num += ni->Items.size();
	if (num < 0 || (tjs_uint)num >= ni->Items.size())
		return TJS_E_MEMBERNOTFOUND;
	CheckObjectClosureRemove(ni->Items[num]);
