...
#include "kirikiri2/core/tjs2/tjsInterCodeExec.h"
...
#define OBJ2 ((objthis) ? (objthis) : (Dispatch2))
	...
	tjs_error TJS_INTF_METHOD
	ClassInstanceInfo(tjs_uint32 flag, tjs_uint num, tTJSVariant *value) { return TJS_E_NOTIMPL; }
...
void tTJSInterCodeContext::DisplayExceptionGeneratedCode(tjs_int codepos,
	...
	for (tjs_int i = 0; i < ra_count; i++) {
		ttstr reg_info = TJS_W("%") + ttstr(i - (MaxVariableCount + VariableReserveCount)) + TJS_W("=") + TJSVariantToReadableString(ra_start[i]);
...
#undef TJS_DEF_VM_P
			...
			case VM_INV:
				TJS_GET_VM_REG(ra, code[1]) =
					TJS_GET_VM_REG(ra, code[1]).Type() != tvtObject ? false : (TJS_GET_VM_REG(ra, code[1]).AsObjectClosureNoAddRef().Invalidate(0, NULL, NULL, ra[-1].AsObjectNoAddRef()) == TJS_S_TRUE);
				code += 2;
				break;

			case VM_CHKINV:
				TJS_GET_VM_REG(ra, code[1]) =
					TJS_GET_VM_REG(ra, code[1]).Type() != tvtObject ? true : TJSIsObjectValid(TJS_GET_VM_REG(ra, code[1]).AsObjectClosureNoAddRef().IsValid(0, NULL, NULL, ra[-1].AsObjectNoAddRef()));
				code += 2;
				break;
			...
			case VM_CHGTHIS:
				TJS_GET_VM_REG(ra, code[1]).ChangeClosureObjThis(TJS_GET_VM_REG(ra, code[2]).AsObjectNoAddRef());
				code += 3;
				break;
...
#define DEBUGGER_EXCEPTION_HOOK                      \
	if (TJSEnableDebugMode && ::IsDebuggerPresent()) \
		TJSDebuggerHook(DBGHOOK_PREV_EXCEPT, NULL, -1, this);
#else // ENABLE_DEBUGGER
#define DEBUGGER_EXCEPTION_HOOK
#endif // ENABLE_DEBUGGER
				...
				case fatExpand:                                                                                \
					args_v_count +=                                                                            \
						TJSGetArrayElementCount(                                                               \
							TJS_GET_VM_REG(ra, (_code)[i * 2 + 1 + 2]).AsObjectNoAddRef());                    \
					break;                                                                                     \
				case fatUnnamedExpand:                                                                         \
					pass_args_count +=                                                                         \
						(numargs > FuncDeclUnnamedArgArrayBase) ? (numargs - FuncDeclUnnamedArgArrayBase) : 0; \
					break;                                                                                     \
				}                                                                                              \
			}                                                                                                  \
