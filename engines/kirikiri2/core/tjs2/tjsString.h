...
#ifndef KIRIKIRI2_CORE_TJS2_TJS_STRING_H
#define KIRIKIRI2_CORE_TJS2_TJS_STRING_H
...
public:
	...
	TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (const tTJSString &rhs)) {
		Ptr = rhs.Ptr;
		if (Ptr)
			Ptr->AddRef();
	}
	TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (tTJSVariantString * vstr)) {
		Ptr = vstr;
		if (Ptr)
			Ptr->AddRef();
	}
	...
	TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (const tTJSStringBufferLength len)) { Ptr = TJSAllocVariantStringBuffer(len.n); }
	...
	//--------------------------------------------------------- conversion --
	TJS_CONST_METHOD_DEF(const tjs_char *, c_str, ()) { return Ptr ? Ptr->operator const tjs_char *() : TJSNullStrPtr; }
	...
	TJS_CONST_METHOD_DEF(bool, StartsWith, (const tTJSString &string)) { return StartsWith(string.c_str()); }
...
#ifdef __CODEGUARD__
		if (!Ptr)
			return 0; // tTJSVariantString::GetLength can return zero if 'this' is NULL
#endif
