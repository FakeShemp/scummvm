...
#ifndef KIRIKIRI2_STD_ALGORITHM_H
#define KIRIKIRI2_STD_ALGORITHM_H
...
template<typename T>
inline T abs(T x) { return ABS(x); }
template<typename T>
inline T min(T a, T b) { return MIN(a, b); }
template<typename T>
inline T max(T a, T b) { return MAX(a, b); }
template<typename T>
inline T clip(T v, T amin, T amax) { return CLIP(v, amin, amax); }
template<typename T>
